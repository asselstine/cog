...
fn encode_reference_ltx(legacy: &[u8]) -> anyhow::Result<Vec<u8>> {
...
fn decode_reference_ltx(reference: &[u8]) -> anyhow::Result<Vec<u8>> {
...
#[doc(hidden)]
pub fn fuzz_validate_reference_ltx(input: &[u8]) {
    let _ = decode_reference_ltx(input);
}
...
fn take_uvarint(input: &[u8], cursor: &mut usize) -> anyhow::Result<u64> {
    let mut value = 0u64;
...
impl Replicator {
    ...
    fn client(&self, generation: u64) -> ReplicaStoreClient {
        ReplicaStoreClient {
...
fn encode_snapshot(path: &FsPath, txid: u64) -> anyhow::Result<Vec<u8>> {
...
fn parse_generation(key: &str) -> Option<u64> {
    key.split('/')
...
mod tests {
    ...
    async fn replica_client_ranges_cleanup_and_empty_compaction() {
        ...
        let client = ReplicaStoreClient {
            store: store.clone(),
            path: "range/g00000000000000000001".into(),
        };
        ...
        fuzz_validate_reference_ltx(&[0xff; 32]);
        let mut cursor = 0;
    ...
    async fn immutable_uploads_are_idempotent_but_never_replace_bytes() {
        ...
        let client = ReplicaStoreClient {
            store,
            path: "app/ltx/g00000000000000000001".into(),
        };
        ...
        let key = client.key(0, TXID(2), TXID(2));
        client
            .store
            .put_opts(
                &key,
    ...
    async fn replicate_incrementally_and_restore_latest_generation() {
        ...
        let interrupted =
            Replicator::new(store.clone(), "app/".into(), interrupted_path.clone(), 2);
