...
import "./index.css";
const body = (v) => new URLSearchParams(v);
...
function Shell({ children, signedIn, signOut }) {
...
function GitHubInstallationComplete() {
...
function Consent() {
  ...
  useEffect(() => {
    fetch(`/api/oauth/consent${window.location.search}`)
      .then(async (response) => {
        ...
        if (!response.ok)
          throw new Error(
            payload.message ||
              payload.error ||
              `Unable to load authorization (${response.status})`,
          );
        setData(payload);
        setIdentity(
...
function App() {
  ...
  async function load() {
    try {
      ...
      if (!r.ok) throw new Error("Unable to load COG");
