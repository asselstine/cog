use super::{catalog::Catalog, tools};
use crate::{authz::InsufficientScope, runtime::CodeRuntime};
...
pub async fn handle_with_options(
    ...
    match req.method.as_str() {
        ...
        "tools/call" => {
            ...
            if !codemode
                && let Some(definition) = tools::by_public_name(name)
                && definition.id != tools::NativeToolId::Execute
            {
                let target = definition.code_target();
                let args = req
                    .params
                    .get("arguments")
                    .cloned()
            ...
            match tools::execute::invoke(runtime, catalog, code.to_owned()).await {
                Ok(v) => {
