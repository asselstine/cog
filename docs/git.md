...
passes through MCP or V8. Agents authenticate with a COG OAuth bearer token, or
with Basic username `cog` and a short-lived repository-bound COG credential.
...
`repositorySelectionUrl`. Open that URL, add the exact repository in GitHub,
and retry the same request. This provider installation step remains separate
from COG's downstream OAuth consent for granting a particular agent read or
write access.

The built-in `git` code-mode catalog target provides `repository_access`,
`repository_list`, `repository_revoke`, and `remote_credentials`. Configure Git
credentials with `credential.useHttpPath=true`; never embed a credential in a
remote URL. Provider branch protection and rulesets remain responsible for pull
request, force-push, status-check, workflow, and signed-commit policy.
...
key; disconnecting removes provider credentials while retaining grants, and
deleting the integration cascades repositories, grants, and derived credentials.
...
The helper implements `get`, `store`, and `erase`, rejects unrelated origins and
non-opaque Git paths, and stores credentials atomically in a mode-0600 runtime
file under `$XDG_RUNTIME_DIR` or a private mode-0700 temporary directory.

MCP hosts that cannot feed the password returned by `remote_credentials` into
the helper can call `credential_bootstrap`. Pass its single-use capability as
`COG_GIT_BOOTSTRAP` and the same OAuth client's access token as
`COG_OAUTH_TOKEN` when invoking the helper. The capability expires after 60
seconds, is bound to that user, OAuth client, repository, and permission
ceiling, and is consumed atomically. Set `COG_GIT_PERMISSION=write` only for a
write grant; it defaults to `read`. The helper exchanges the capability over
`POST /git/bootstrap` and stores only the resulting 15-minute credential.
...
For incident response, disable or disconnect the integration, revoke affected
repository grants, rotate the App key in GitHub, and update COG's encrypted
`privateKey`. Configuration changes, disconnect, and deletion invalidate cached
installation tokens. JWTs and installation tokens are memory-only.
